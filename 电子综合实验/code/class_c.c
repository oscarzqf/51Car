...
sbit KEY3 = P3^6;		//定义按键3，计数器启动按钮，对应核心板上K3

unsigned char cnt = 10;		//定义报警门限值	
unsigned char counter_1ms=0;//1ms计时
unsigned int sec=0;//0.01秒计数
unsigned char counter1_1ms=0;//报警用计时
bit timer_start_flag = 0;	//定义标志位,控制计时暂停与继续，默认0暂停
//=====================================	
...
if(timer_start_flag){
		...
		if(counter_1ms == 10)
		{
			...
			DispBuf[4] = Tab[sec % 10];			//查表取出显示数字对应的段码，存入显示缓冲器数组
			DispBuf[5] = Tab[sec / 10 % 10] ; 	// "| 0x80"可以使该位带小数点显示
			DispBuf[6] = Tab[sec / 100%10]| 0x80;
			DispBuf[7] = Tab[sec / 1000];
